...
def list_flagged_for_review(
    ...
    Audit fix (High, dimension 1/IDOR): previously accepted a caller-
    suppliable hospital_profile_id -- any of the three roles could read
    another hospital's flagged-override queue. Always scoped to the
    caller's own hospital now."""
    return [
        HumanDecisionResponse.model_validate(d)
        for d in store.list_flagged_for_review(current_user.hospital_profile_id)
    ]
...
def get_override_monitoring(
    ...
    Audit fix (High, dimension 1/IDOR): always scoped to the caller's own
    hospital now, same reasoning as list_flagged_for_review above."""
