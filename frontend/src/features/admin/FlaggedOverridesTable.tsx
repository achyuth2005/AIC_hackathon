...
export const FlaggedOverridesTable: React.FC<FlaggedOverridesTableProps> = ({
  ...
  return (
    <Card className="bg-slate-900 border-amber-900/60 text-left shadow-xl">
      <CardHeader className="pb-3 flex flex-row items-center justify-between">
        ...
        <span className="text-[10px] font-mono font-bold bg-amber-950 text-amber-300 px-2.5 py-0.5 rounded-full border border-amber-800">
      ...
      <CardContent>
        {isLoading ? (
          ...
          <Table>
            ...
            <TableBody>
              {overrides.map((d) => (
                <TableRow key={d.decision_id} className="bg-amber-950/10">
                  <TableCell>
                  ...
                  <TableCell className="text-xs font-mono text-slate-300">
                    ...
                    <div className="text-[10px] text-slate-500">{formatClock(d.timestamp, true)}</div>
                  </TableCell>
