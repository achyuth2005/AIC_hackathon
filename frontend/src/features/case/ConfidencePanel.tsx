...
export const ConfidencePanel: React.FC<ConfidencePanelProps> = ({
  ...
  return (
    <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-card space-y-3 text-left">
      ...
      {shouldAbstain && (
        <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-900 flex items-start gap-2.5">
          ...
          <div>
            <div className="font-bold text-amber-800">Confidence Engine Abstaining:</div>
      ...
      {confidenceReasons && confidenceReasons.length > 0 && (
        <div className="space-y-1.5 pt-1">
          <div className="text-[11px] font-semibold text-slate-500">Clinical Signals:</div>
          <ul className="space-y-1">
            {confidenceReasons.map((reason, idx) => (
              <li
                key={idx}
                className="text-xs text-slate-700 flex items-start gap-2 bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-200/80"
              >
