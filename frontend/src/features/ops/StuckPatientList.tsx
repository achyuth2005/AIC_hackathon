...
export const StuckPatientList: React.FC = () => {
  ...
  return (
    <Card className="border-rose-200 text-left shadow-card-lg">
      <CardHeader className="pb-3 flex flex-row items-center justify-between">
        <CardTitle className="text-base flex items-center gap-2.5 text-rose-800">
          ...
          <Table>
            ...
            <TableBody>
              {stuckPatients!.map((p, idx) => (
                <TableRow key={`${p.case_id}-${idx}`} className="bg-rose-50">
                  ...
                  <TableCell>
                    <span className="px-2 py-0.5 rounded bg-white text-slate-600 border border-slate-200 text-[10px] font-mono font-bold">
                      {p.route_to}
                    </span>
                  </TableCell>
                  ...
                  <TableCell className="text-right">
                    <Link
                      to={`/cases/${p.case_id}`}
                      className="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-700 font-mono font-semibold"
                    >
